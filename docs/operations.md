...
6. Configure `OCIR_REGISTRY` and `OCI_NAMESPACE` as GitHub variables. Add a dedicated CI user's `OCIR_USERNAME` and auth token as GitHub secrets.
...
The hosted GitHub runner tests the project and pushes `linux/amd64` and `linux/arm64` images tagged with the Git SHA. After CI succeeds on `main`, the production runner calls `scripts/deploy.sh`. The script saves the previous image environment, applies Alembic, starts Compose, checks `https://<domain>/health/ready`, and restores the previous image tags on failure.
