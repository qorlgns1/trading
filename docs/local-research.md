...
Open **전략 실험실** to choose a baseline, add up to three challengers, split training and validation periods, run walk-forward and stress checks, and explore at most 100 controlled sensitivity combinations. A completed strategy keeps cause analysis, trade quality, integrity, HTML/JSON/CSV, and Parquet ledgers. The current snapshot still uses listed securities and therefore displays a survivorship-bias warning. Open **후보 이력** for daily candidate changes. **포워드 포트폴리오** supports one baseline and three experimental active accounts, freezes each promoted strategy, and waits for the next completed review before creating orders. These local-only APIs return `403` in `public_demo`.
